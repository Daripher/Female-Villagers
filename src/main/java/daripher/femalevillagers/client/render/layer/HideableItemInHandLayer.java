...
public class HideableItemInHandLayer<T extends AbstractIllager> extends ItemInHandLayer<T, IllagerModel<T>> {
	...
	public HideableItemInHandLayer(RenderLayerParent<T, IllagerModel<T>> parent, ItemInHandRenderer itemInHandRenderer, Predicate<T> shouldShowItemPredicate) {
		super(parent, itemInHandRenderer);
		this.shouldShowItemPredicate = shouldShowItemPredicate;
	}

	@Override
	public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTicks,
			float entityAge, float netHeadYaw, float headPitch) {
